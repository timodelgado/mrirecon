...
class RegPolicyConfig:
    ...
    tv_sample_K: int = 4096              # number of TV samples (global aggregate target)
...
class RegPolicy:
    ...
    def prepare_collection(self, ws, regm) -> None:
        ...
        # Provide TV sampling knobs to the manager (read via getattr with defaults)
        if need_tv_q:
            sb.tv_percentile = float(self.cfg.eps_percentile)
            sb.tv_sample_K = int(getattr(self.cfg, "tv_sample_K", 4096))

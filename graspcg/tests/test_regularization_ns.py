...
def device_list():
    ...
    if torch.cuda.is_available():
        # keep a single GPU to avoid spurious multi-GPU CI failures
...
class FakeWS:
    """
    Minimal workspace for RegManager/mapping:
    ...
    def __init__(self, x_shards, *, stats=None, extras=None, device=None):
        ...
        # New canonical buffer names
        self.buffers = {
            "var":  self.x_shards,
            "grad": [torch.zeros_like(xi) for xi in self.x_shards],
...
def test_tvnd_energy_grad_constant_zero(device):
    ...
    assert E.dtype == ws.get("grad", 0).real.dtype
    assert float(E.detach().cpu()) == pytest.approx(0.0, abs=1e-6)
    assert torch.allclose(ws.get("grad", 0), torch.zeros_like(ws.get("grad", 0)))

@pytest.mark.parametrize("device", device_list())
def test_manager_sharded_vs_whole_energy_temporal_halo(device):
    # Vary over sharded axis (B) so halos are needed.
    B, C, H, W = 8, 1, 4, 4
...
@pytest.mark.parametrize("device", device_list())
def test_mapped_temporal_basis_updates_gV_not_var(device):
    # Build u = U @ V then TV on spatial dims; gradient should land in g_V, not g(var)
    T, K = 6, 3
    C, H, W = 1, 4, 4
    U = torch.randn(T, K)
    ...
    E = regm.energy_and_grad(ws)
    # var-grad should be zero (param-only mapping)
    assert torch.allclose(ws.get("grad", 0), torch.zeros_like(ws.get("grad", 0)))
    assert torch.allclose(ws.get("grad", 1), torch.zeros_like(ws.get("grad", 1)))
...
@pytest.mark.parametrize("device", device_list())
def test_stats_board_probe_quantile_collection(device):
    B, C, H, W = 8, 1, 8, 8
    x = (torch.rand((B, C, H, W), device=device) - 0.5).to(torch.complex64)
    sb = StatsBoard()
    sb.enable("probe_quantile", True)
    sb.probe_percentile = 0.9
    sb.probe_sample_K = 128
    ...
    q = sb.read_scalar(f"probe/q/{tv.name}")
    assert q >= 0.0
...
def test_add_diag_behavior_identity_vs_mapped(device):
    ...
    # Mapped (TemporalBasis): image diag should not change (no diag_V provided)
    T, K = B, 2
    U = torch.randn(T, K)
...
def test_graph_diag_degree(device):
    ...
    class WS:
        def __init__(self): self._bufs={"var":[x], "grad":[g], "diag":[D]}
...
def test_graph_energy_grad_matches_fd(device):
    ...
    ctx = RegContext(
        ...
        ws=type("W", (), {
            ...
            "get": lambda self,k,i: {"var":x,"grad":g,"diag":D}[k] if k in {"var","grad","diag"} else {"x":x,"g":g,"diag":D}[k],

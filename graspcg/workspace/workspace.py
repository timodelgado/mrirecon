...
from ..compat.names import CANONICAL, ALIAS_TO_CANON
...
class VarSlotNames:
    """Names of buffers that implement a logical variable."""
    val: str                 # required (value of the variable)
    grad: str | None = None  # optional
    diag: str | None = None  # optional (preconditioner diagonal)
    direction: str | None = None  # optional (search direction)
...
class Workspace:
    ...
    def _install_aliases(self) -> None:
        """
        Ensure canonical names and their aliases point to the same storage.
        Works for both global buffers (Tensor) and per_shard buffers (list[Tensor]).
        """
        # Find any present key among canon+aliases; bind all to the first found.
        for canon, aliases in CANONICAL.items():
            group = [canon] + list(aliases)
            present_key = next((k for k in group if k in self._bufs), None)
            if present_key is None:
                continue
            base_obj = self._bufs[present_key]
            for k in group:
                self._bufs[k] = base_obj
    ...
    def mark_x_written(self) -> None:
        """
        Publish an event per device indicating that writes to ws.x are complete
        on the stream that performed them (typically the default stream).
        Halo reads on per-device streams should wait on these events.
        """
        self._x_events = {}
        for sh, i in self.iter_shards():
            dev = sh.device
            if getattr(dev, "type", "cpu") != "cuda":
                continue
            # Record an event on the current stream of that device (default stream by default)
            with torch.cuda.device(dev):
                s = torch.cuda.current_stream(device=dev)
                e = torch.cuda.Event()
                s.record_event(e)
                self._x_events[dev.index] = e

    def wait_x_ready(self, dev: torch.device, neighbors: list[torch.device] | None = None) -> None:
        """
        On the per-device work stream, wait for the 'x-written' events from this
        device and the provided neighbor devices (for halo exchange safety).
        """
        if getattr(dev, "type", "cpu") != "cuda":
            return
        s = self.arena.stream_for(dev)
        # Wait on our own device event as well as neighbor devices
        to_wait = [dev]
        if neighbors:
            to_wait += [nb for nb in neighbors if getattr(nb, "type", "") == "cuda"]
        evs = getattr(self, "_x_events", {})
        for nb in to_wait:
            ev = evs.get(nb.index) if hasattr(nb, "index") else None
            if ev is not None:
                s.wait_event(ev)
    ...
    def __init__(self,
        ...
        # problem-agnostic constants (optional):
        ...
        # expose canonical <-> alias names so both work everywhere
        try:
    ...
    @property
    def data(self):
        """Acquired signal (canonical name). Alias to `y`."""
        return self.y

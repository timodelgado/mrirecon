...
class DeviceArena:
    ...
    def request(self,
        ...
        if idx < len(sizes):
            base = bufs.pop(idx); sizes.pop(idx)
            self._touch[id(base)] = time.time()
            return base[:numel]                # return a view of the base
        ...
        base = torch.empty(n_alloc, dtype=dtype, device=loc,
                        pin_memory=(loc.type == "cpu" and self.pin_memory))
        self._touch[id(base)] = time.time()
        return base[:numel]
    ...
    def release(self, tensor: torch.Tensor):
        base, cap = self._base_and_capacity(tensor)
        key = (base.device, base.dtype)
        sizes, bufs = self._pools[key]
        idx = bisect.bisect_left(sizes, cap)
        sizes.insert(idx, cap)
        bufs.insert(idx, base)
        self._pools[key] = self._Pool(sizes, bufs)
        self._touch[id(base)] = time.time()
        # enforce per-(dev,dtype) byte caps if configured
        if self.max_bytes is not None:
            self._evict_over_cap(key)
    ...
    def _base_and_capacity(self, t: torch.Tensor) -> tuple[torch.Tensor, int]:
        base = getattr(t, "_base", None)
        base = base if isinstance(base, torch.Tensor) else t
        # capacity in elements, not just current view
        cap = int(base.numel())
        return base, cap

...
from ..numerics.utils import reduce0d, dot_real0
...
class Objective:
    ...
    def f_g_tensor(self, ws, t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._use_sharded:
            ...
            for sh, i in ws.iter_shards():
                ...
                with ctx_mgr:
                    ...
                    # residual + energy (compiled) — use per‑shard caches + matching y slice
            ...
            f_data = self._reduce0d(f_data_dev, prefer_device=t.device, dtype=dtype_r)
            f_data.mul_(0.5)
        ...
        for sh, i in ws.iter_shards():
            g, d = ws.bind(i, "g", "dx")
            dev = g.device
            gd = dot_real0(g, d)          # instead of self._ker_dot_real(...)
            gdot = reduce0d(gdot_dev, prefer_device=ws.primary_device, dtype=dtype_r)
            f_data = reduce0d(f_data_dev, prefer_device=t.device, dtype=dtype_r); f_data.mul_(0.5)
            if dev in gdot_dev:
        ...
        # record total (respects LS scoping if active)
        sb = getattr(ws, "stats", None)
        if sb is not None:
            sb.scalar_slot("f_total", f_total.device, f_total.dtype).add_(f_total)

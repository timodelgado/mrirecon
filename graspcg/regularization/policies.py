...
class RegPolicy:
    ...
    def _rebuild_preconditioner(self, ws, regm) -> None:
        """
        Delegate to RegManager.refresh_preconditioner for a single source of truth.
        """
        try:
            regm.refresh_preconditioner(
                ws,
                mode="auto",
                ema_alpha=self.cfg.precond_ema_alpha,
                floor=1e-12,
                mix_data=self.cfg.precond_base - 1.0 if hasattr(self.cfg, "precond_base") else 0.0,
                verbose=False,
            )
        except Exception:
            pass
